...
# This script tests the Ansible playbooks on both Ubuntu and Arch Linux
...
main() {
    ...
    echo "This will test on both Ubuntu (simulating macOS) and Arch Linux"
    echo ""
    ...
    if [ $ubuntu_failed -eq 0 ] && [ $arch_failed -eq 0 ]; then
...
case "$1" in
    --help|-h)
        ...
        echo ""
        echo "Without options, tests both platforms"
        exit 0
        ;;
    --cleanup|-c)
        cleanup
